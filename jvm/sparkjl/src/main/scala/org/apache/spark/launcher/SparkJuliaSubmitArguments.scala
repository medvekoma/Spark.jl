...
import org.apache.spark.api.julia.JuliaRunnerOrig
import org.apache.spark.deploy.SparkSubmit
import org.apache.spark.runner.JuliaRunner
//import org.apache.spark.deploy.csharp.CSharpRunner
import org.apache.spark.util.Utils
import org.apache.spark.julia.{Utils => JuliaUtils}
...
object SparkJuliaSubmitArguments {

  val juliaRunnerClass: String = "org.apache.spark.runner.JuliaRunner"
//  var exitFn: Int => Unit = (exitCode: Int) => CSharpUtils.exit(exitCode)
  ...
  val exitFn = null //TODO fix me

  def main(args: Array[String]): Unit = {
    val submitArguments = new SparkJuliaSubmitArguments(args, sys.env, exitFn, printStream)
...
/**
  ...
  */
class SparkJuliaSubmitArguments(
                               args: Seq[String],
  ...
  import SparkJuliaSubmitArguments._

  val MAIN_EXECUTABLE: String = "--jl"
  ...
  var sparkJuliaJarPath: String =
    new File(JuliaRunner.getClass.getProtectionDomain.getCodeSource.getLocation.getPath).getPath
...
//  }
  ...
  private def validateSubmitArguments(): Unit = {
    ...
    if (mainExecutable == null || !mainExecutable.toLowerCase().endsWith(".jl")) {
  ...
  private def concatLocalCmdOptions(): Unit = {
    ...
    cmd += s" --class $juliaRunnerClass $sparkJuliaJarPath $primaryResource"
  ...
  private def concatCmdOptions(): Unit = {
    ...
    master match {
      ...
      case m if m.toLowerCase.startsWith("spark://") && deployMode == "cluster" => {
        ...
        cmd += (s" --files $files --class $juliaRunnerClass $remoteSparkCLRJarPath" +
      ...
      case _ => {
        ...
        deployMode match {
          ...
          case "cluster" => {
            cmd += (s" --class $juliaRunnerClass $sparkJuliaJarPath "
  ...
  private def zipPrimaryResource(): File = {
    ...
    primaryResource match {
      ...
      case pr if new File(pr).isDirectory => {
        ...
        JuliaUtils.zip(new File(primaryResource), zippedResource)
      }

      case pr if pr.endsWith(".exe") => {
        zippedResource = new File(System.getProperty("java.io.tmpdir"),
          System.currentTimeMillis() + ".zip")
        JuliaUtils.zip(new File(primaryResource).getParentFile, zippedResource)
      }
