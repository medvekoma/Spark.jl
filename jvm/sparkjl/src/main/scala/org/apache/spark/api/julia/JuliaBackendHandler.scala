...
import org.apache.spark.api.julia.SerDe._
...
class JuliaBackendHandler(server: JuliaBackend) extends SimpleChannelInboundHandler[Array[Byte]]
  with Logging{
    ...
    if (objId == "SparkCLRHandler") {
      methodName match {
        ...
        case "connectCallback" =>
          ...
          JuliaBackend.callbackPort = port
          writeInt(dos, 0)
          writeType(dos, "void")
        case "closeCallback" =>
          ...
          do {
            socket = JuliaBackend.callbackSockets.poll()
            if (socket != null) {
          ...
          JuliaBackend.callbackSocketShutdown = true
